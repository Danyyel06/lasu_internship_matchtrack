content = """import { Link, Outlet, useLocation, useNavigate } from 'react-router-dom';
...
export default function SupervisorLayout() {
  ...
  const navigation = [
    ...
    { name: 'Framework Approvals', href: '/supervisor/frameworks', icon: '??' },
  ];
    ...
    <div className="flex h-screen bg-neutral-50 overflow-hidden font-sans">
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-neutral-200 pb-safe z-30">
        <div className="flex justify-around items-center h-16">
          <Link to="/supervisor/dashboard" className={`flex flex-col items-center justify-center w-full h-full ${location.pathname.includes('/supervisor/dashboard') ? 'text-blue-600' : 'text-neutral-500'}`}>
            <span className="text-xl">??</span>
            <span className="text-[10px] font-medium mt-1">Home</span>
          </Link>
          <Link to="/supervisor/interns" className={`flex flex-col items-center justify-center w-full h-full ${location.pathname.includes('/supervisor/interns') ? 'text-blue-600' : 'text-neutral-500'}`}>
            <span className="text-xl">??</span>
            <span className="text-[10px] font-medium mt-1">Interns</span>
          </Link>
        </div>
      </nav>
    </div>
...
"""
with open('frontend/src/layouts/SupervisorLayout.tsx', 'w', encoding='utf-8') as f:
    f.write(content)
