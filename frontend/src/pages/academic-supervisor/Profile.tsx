...
export default function AcademicSupervisorProfile() {
  ...
  const fetchProfile = async () => {
    try {
      ...
      const res = await axios.get('http://localhost:8000/api/v1/academic-supervisors/profile', {
  ...
  const handleUpdate = async () => {
    try {
      const token = localStorage.getItem('access_token');
      await axios.put('http://localhost:8000/api/v1/academic-supervisors/profile', editForm, {
        headers: { Authorization: `Bearer ${token}` }
      });
      setIsEditing(false);
      fetchProfile();
